...
ID_COLOR = {1: 'brown', 2: 'magenta', 3: 'orange', 4: 'gray', 5: 'red', 6: 'blue', 7: 'cyan', 8: 'green', 9: 'salmon',
            10: 'yellow'}
NUM_WALL_CORNERS = 13
NUM_CORNERS = 21
NUM_ICONS = 7
...
class ColorPalette:
    def __init__(self, num_colors):
        ...
        if num_colors > self.colorMap.shape[0]:
            self.colorMap = np.random.randint(255, size=(num_colors, 3))
            pass
    ...
    if segmentations.ndim == 2:
        ...
        if blackThreshold > 0:
            segmentations = np.concatenate(
                [segmentations, np.ones((segmentations.shape[0], segmentations.shape[1], 1)) * blackThreshold], axis=2)
            blackIndex = segmentations.shape[2] - 1
            pass

        numColors = max(numColors, segmentations.shape[2] + 2)
        pass
    randomColor = ColorPalette(numColors).get_color_map()
    if blackIndex >= 0:
...
def drawPoints(filename, width, height, points, backgroundImage=None, pointSize=5, pointColor=None):
    colorMap = ColorPalette(NUM_CORNERS).get_color_map()
...
def drawLines(filename, width, height, points, lines, lineLabels=[], backgroundImage=None, lineWidth=5, lineColor=None):
    colorMap = ColorPalette(len(lines)).get_color_map()
...
def drawRectangles(filename, width, height, points, rectangles, labels, lineWidth=2, backgroundImage=None,
                   rectangleColor=None):
    colorMap = ColorPalette(NUM_ICONS).get_color_map()
    if backgroundImage is None:
        image = np.ones((height, width, 3), np.uint8) * 0
    else:
        image = backgroundImage
...
def calcLineDirectionPoints(points, line):
    ...
    if isinstance(point_2[0], tuple):
        point_2 = point_2[0]
        pass
    return calc_line_direction((point_1, point_2))
...
def bb_to_img(bbs, graphs, room_to_sample, triple_to_sample, boundary_bb=None, max_num_nodes=10, im_size=512,
              disc_scores=None):
    imgs = []
...
def rectangle_renderer(theta, im_size=64):
    ...
    # conditions
    cond_1 = torch.min(torch.cat([F.relu(ys - theta[:, 1].view(-1, 1, 1)).unsqueeze(-1),
                                  torch.ones((theta.shape[0], im_size, im_size, 1)).cuda()], -1), -1)[0] * \
             torch.min(torch.cat([F.relu(theta[:, 3].view(-1, 1, 1) - ys).unsqueeze(-1),
                                  torch.ones((theta.shape[0], im_size, im_size, 1)).cuda()], -1), -1)[0]

    cond_2 = torch.min(torch.cat([F.relu(xs - theta[:, 0].view(-1, 1, 1)).unsqueeze(-1),
                                  torch.ones((theta.shape[0], im_size, im_size, 1)).cuda()], -1), -1)[0] * \
             torch.min(torch.cat([F.relu(theta[:, 2].view(-1, 1, 1) - xs).unsqueeze(-1),
                                  torch.ones((theta.shape[0], im_size, im_size, 1)).cuda()], -1), -1)[0]

    # lines
    line_1 = (F.relu(torch.ones((theta.shape[0], im_size, im_size)).cuda() - torch.abs(
        xs - torch.ones((theta.shape[0], im_size, im_size)).cuda() - theta[:, 0].view(-1, 1, 1))) * cond_1).view(-1,
                                                                                                                 im_size,
                                                                                                                 im_size,
                                                                                                                 1)  # top
    line_2 = (F.relu(torch.ones((theta.shape[0], im_size, im_size)).cuda() - torch.abs(
        xs + torch.ones((theta.shape[0], im_size, im_size)).cuda() - theta[:, 2].view(-1, 1, 1))) * cond_1).view(-1,
                                                                                                                 im_size,
                                                                                                                 im_size,
                                                                                                                 1)  # bottom
    line_3 = (F.relu(torch.ones((theta.shape[0], im_size, im_size)).cuda() - torch.abs(
        ys - theta[:, 1].view(-1, 1, 1))) * cond_2).view(-1, im_size, im_size, 1)  # left
    line_4 = (F.relu(torch.ones((theta.shape[0], im_size, im_size)).cuda() - torch.abs(
        ys - theta[:, 3].view(-1, 1, 1))) * cond_2).view(-1, im_size, im_size, 1)  # right
...
def checkpoint(real_room_bb, fake_room_bb, nodes, triples, room_to_sample, triple_to_sample, generator, exp_folder,
    ...
    for _ in range(10):
        ...
        # plot images
        fake_room_bb_partial = generator(z_partial, [nodes_partial, triples_partial], room_to_sample_partial,
                                         boundary=boundary_bb_partial)
        fake_imgs_tensor = bb_to_img(fake_room_bb_partial.data, [nodes_partial, triples_partial],
                                     room_to_sample_partial, \
    ...
    layouts_imgs_tensor_filtered = torch.stack(layouts_imgs_tensor_filtered).contiguous().view(-1, 3, out_imsize,
                                                                                               out_imsize)
